...
class Kernel:
    ...
    def respond(self, input, sessionID = _globalSessionID):
        ...
        self.question = input
	if self.question.encode('latin').find("memorize") > -1 :
    ...
    def _processMemorize(self):
                  """ - when "memorize" keyword is heard, sentence is learnt and written as XML in neo.aiml
		  ...
		  Exemple in one session :
			...
			- what is a dog
				- I don't know
			- memorize a dog is an animal
				- Ok, I note that in my BDD
			- what is a dog
				- an animal
                  """
                  if self.question.find("is") > -1 :
                    verb = " is "
                  if self.question.find("means") > -1 :
                    verb = " means "
                  if self.question.find("are") > -1 :
                    verb = " are "
                  else :
                    pass
                  a = self.question.encode("latin").replace("memorize","").split(verb)
                  print "\n---------------------------------------------------------"
                  print "ALIMENTATION IN BDD : "
                  print "question : "+unicode(a[0], "ISO-8859-1").upper()+" ; ANSWER : "+unicode(a[1], "ISO-8859-1").upper()
                  print "---------------------------------------------------------\n"
                  ...
                  readFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_en/neo.aiml", "r")
                  readFile
                  operation = (readFile.readlines())
                  del operation[-1:]
                  writeFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_en/neo.aiml", "w")
                  writeFile.write("".join(operation))
                  writeFile.close()
                  
                  # create xml section
                  appendFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_en/neo.aiml", "a")
                  appendFile

                  if verb == " is ":
		    appendFile.write("\n  <category>\n    <pattern>WHO IS"+ Q+"</pattern>\n    <template>It's "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>WHAT IS"+ Q+"</pattern>\n    <template>It's "+ R+"</template>\n  </category>\n</aiml>")

                  if verb == " means ":
		    appendFile.write("\n  <category>\n    <pattern>WHAT MEANS"+ Q+"</pattern>\n    <template>It means "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>what is"+ Q+"</pattern>\n    <template>It is "+ R+"</template>\n  </category>\n</aiml>")

                  if verb == " are ":
		    appendFile.write("\n  <category>\n    <pattern>WHO ARE"+ Q+"</pattern>\n    <template>They are "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>WHAT ARE"+ Q+"</pattern>\n    <template>It's "+ R+"</template>\n  </category>\n</aiml>")
                  appendFile.close()
