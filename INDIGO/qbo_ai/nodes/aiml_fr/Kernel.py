...
class Kernel:
    ...
    def __init__(self):
        ...
        self._elementProcessors = {
            ...
            "day":          self._processDay,
            "hour":         self._processHour,
    ...
    def respond(self, input, sessionID = _globalSessionID):
        ...
        self.question = input
	if self.question.encode('latin').find("mémorise") > -1 :
          self._processMemorize()
        else :
          pass
	...
	a = time.strftime('%a %d %b %Y')
	b = a.replace("Sun","Dimanche").replace("Mon","Lundi").replace("Tue","Mardi").replace("Wed","Mercredi").replace("Thu","Jeudi").replace("Fri","Vendredi").replace("Sat","Samedi").replace("Jan","Janvier").replace("Feb","Février").replace("Mar","Mars").replace("Apr","Avril").replace("May","Mai").replace("Jun","Juin").replace("Jui","Juillet").replace("Aug","Aout").replace("Sep","Septembre").replace("Oct","Octobre").replace("Nov","Novembre").replace("Dec","Décembre")
        return b


    # <day>
    def _processDay(self, elem, sessionID):
        a = now.strftime('%a')
        b = a.replace("Sun","Dimanche").replace("Mon","Lundi").replace("Tue","Mardi").replace("Wed","Mercredi").replace("Thu","Jeudi").replace("Fri","Vendredi").replace("Sat","Samedi")
        return b


    # <hour>
    def _processHour(self, elem, sessionID):     
        return now.strftime("%H heures %M")
    ...
    # memorize process : 
    def _processMemorize(self):
                  """ - when "mémorise" keyword is heard, sentence is learnt and written as XML in neo.aiml
	              - response is done with memorise.aiml, and use random for better natural
                      - Right after, neo.aiml is reloaded

		  Exemple in one session :
                  ----------------------
			- what is a dog
				- I don't know
			- mémorise a dog is an animal
				- Ok, I note that in my BDD
			- what is a dog
				- an animal
                  """
                  if self.question.find("est") > -1 :
                    verb = " est "
                  if self.question.find("signifie") > -1 :
                    verb = " signifie "
                  if self.question.find("sont") > -1 :
                    verb = " sont "
                  else :
                    pass
                  a = self.question.encode("latin").replace("mémorise","").split(verb)
                  print "\n---------------------------------------------------------"
                  print "ALIMENTATION EN BDD : "
                  print "question : "+unicode(a[0], "ISO-8859-1").upper()+" ; REPONSE : "+unicode(a[1], "ISO-8859-1").upper()
                  print "---------------------------------------------------------\n"
                  #Q = a[0].decode("utf8").upper().encode('latin1')
                  #R = a[1].decode('utf-8').lower().encode('latin1')
                  #Q = a[0].upper()#.replace("é","e").replace("è","e").replace("ê","e").replace("à","a").replace("â","a").replace("ù","u").replace("ô","o").replace("ë","e").upper()
                  Q = unicode(a[0], "ISO-8859-1").upper()
                  Q = Q.encode("ISO-8859-1")
                  R = a[1].lower()
                  # open text and erase last </aiml> tag, however it won't work
                  readFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_fr/neo.aiml", "r")
                  readFile
                  operation = (readFile.readlines())
                  del operation[-1:]
                  writeFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_fr/neo.aiml", "w")
                  writeFile.write("".join(operation))
                  writeFile.close()
                  
                  # create xml section
                  appendFile=open("/home/neo/catkin_ws/src/qbo_ai/ai_fr/neo.aiml", "a")
                  appendFile

                  if verb == " est ":
		    appendFile.write("\n  <category>\n    <pattern>QUI EST"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>QUI C EST"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>C EST QUI"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>QU EST CE QUE"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>QU EST CE QUE C EST"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>C EST QUOI"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>\n</aiml>")

                  if verb == " signifie ":
		    appendFile.write("\n  <category>\n    <pattern>QUE SIGNIFIE"+ Q+"</pattern>\n    <template>Ca signifie "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>QUE VEUT DIRE"+ Q+"</pattern>\n    <template>Ca veut dire "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>CA VEUT DIRE QUOI"+ Q+"</pattern>\n    <template>Ca signifie "+ R+"</template>\n  </category>\n</aiml>")

                  if verb == " sont ":
		    appendFile.write("\n  <category>\n    <pattern>QUI SONT"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>QUE SONT"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>")
		    appendFile.write("\n  <category>\n    <pattern>C EST QUOI"+ Q+"</pattern>\n    <template>C'est "+ R+"</template>\n  </category>\n</aiml>")

                  appendFile.close()
                  print "Information added in my BDD"
                  return
...
if __name__ == "__main__":
    ...
    _testTag(k, 'day', 'test day')
    _testTag(k, 'hour', 'test hour')
    _testTag(k, 'memorize', 'test memorize')
